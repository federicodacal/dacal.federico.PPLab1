...
static int isTelephone(char* string);
static int esDNI(char* string);
...
static int esDescripcion(char* cadena,int longitud)
{
	...
	if(cadena != NULL && longitud > 0)
	{
		for(i=0 ; cadena[i] != '\0' && i < longitud; i++)
		{
			if((cadena[i] < 'A' || cadena[i] > 'Z' ) && (cadena[i] < 'a' || cadena[i] > 'z' ) && (cadena[i]!=' ') && (cadena[i]!='.') && (cadena[i]<'0' || cadena[i]>'9'))
			{
				retorno = 0;
				break;
			}
...
int validarSexo(char* str)
{
    ...
    if(str!=NULL){
        if(!stricmp(str,"masculino") || !stricmp(str,"femenino")){
            retorno=0;
        }
    }
    return retorno;
...
int utn_getTelefono(char* pResultado, char* mensaje, char* mensajeError, int minSize, int maxSize, int reintentos)
{
    ...
    if(mensaje!=NULL && mensajeError!=NULL && minSize<maxSize && reintentos>=0 && pResultado!=NULL)
    {
        do
        {
            if(!getString(bufferStr,sizeof(bufferStr)))
            {
                if(isTelephone(bufferStr))
                {
                    strncpy(pResultado,bufferStr,maxSize);
                    retorno=0;
                    break;
                }
                else
